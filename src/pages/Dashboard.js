...
const Dashboard = ({ userId, userProfile, db, setError, setSuccess, currentAppId, navigateToView, theme }) => {
    ...
    useEffect(() => {
        ...
        }, err => { console.error("Leads count error:", err); });
            ...
            >
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <div className="text-3xl font-bold">{isLoadingCard ? '...' : value}</div>
                ...
                {punchStatus.isLoading ? (
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4">
                        {punchStatus.status === 'out' ? (
                            <button onClick={() => handlePunch('punch-in')} className="w-full sm:w-auto bg-green-500 hover:bg-green-600 text-white font-semibold py-3 px-6 rounded-lg shadow-md flex items-center justify-center">
                                <LogIn size={20} className="mr-2" /> Punch In
                            </button>
                        ) : (
                            <button onClick={() => handlePunch('punch-out')} className="w-full sm:w-auto bg-red-500 hover:bg-red-600 text-white font-semibold py-3 px-6 rounded-lg shadow-md flex items-center justify-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                <div className="lg:col-span-1 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                    ...
                    {isLoading ? <LoadingSpinner text="Loading chart..." /> : dashboardData.leadStatusData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={250}>
                            <PieChart>
                                <Pie data={dashboardData.leadStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label>
                ...
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                    ...
                    {isLoading ? <LoadingSpinner text="Loading chart..." /> : dashboardData.dealStageData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={250}>
                            <BarChart data={dashboardData.dealStageData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                                <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} />
                                <XAxis dataKey="name" stroke={theme === 'dark' ? '#9ca3af' : '#4b5563'} />
                                <YAxis allowDecimals={false} stroke={theme === 'dark' ? '#9ca3af' : '#4b5563'} />
                                <RechartsTooltip content={<CustomTooltip />} />
                                <Legend iconSize={10} />
                                <Bar dataKey="value" name="Deals" fill="#34d399" radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                    ...
                    {isLoading ? <LoadingSpinner text="Loading chart..." /> : dashboardData.activityTypeData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={250}>
                            <PieChart>
                                <Pie data={dashboardData.activityTypeData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label>
